...
class ScaleTypeUtil {
    ...
    companion object {
        private const val TAG = "${EvaConstant.TAG}.ScaleTypeUtil"
    }
    ...
    fun getRealSize(): Pair<Int, Int> {
        ...
        ELog.i(TAG, "get real size (${size.first}, ${size.second})")
        return size
    }
    ...
    private fun getCurrentScaleType(): IScaleType {
        ...
        return if (tmpScaleType != null) {
            ELog.i(TAG, "custom scaleType")
            tmpScaleType
        } else {
            ELog.i(TAG, "scaleType=$currentScaleType")
